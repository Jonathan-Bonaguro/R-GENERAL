...
### Your answer here
#"set.seed" is setting the seed in order to get consistent results from the next sample command.
#"sample" is creating a vector of length 25 by taking random numbers between 1 and 9. 
#"matrix" is crating a maxtrix with 5 columns from the vector one_num
...
### Your code here
apply(one_matrix, 1, function(x) diff(range(x)))

# (4) Calculate the sum of all numbers smaller than 5 for each column. (2pts)

### Your code here
apply(one_matrix, 2, function(x) sum(x[x<5]))
...
#sapply returned a vector telling me that all of the classes were numeric, and lapply returned a list
...
# The code doesn't work because the log function expects a vector input and we gave it a list. 
...
### Your code here
mapply(function(x,y) log(sum(x+y)), list_1, list_2)
...
### Your code here
rapply(list_3, log, classes = "integer")
...
### Your code/answer here
simple.lm=lm(iris$Petal.Length~iris$Petal.Width)
simple.lm

# The intercept tells us that with a Petal Width of 0, the model predicts that the petal length will be 1.084.
# The next coefficient, "iris$Petal.Width" tells us that for every one unit that Petal Width increases, Petal Length will increase by 2.230.
...
# This plot shows us the relationship between petal width and petal lenght. The regression line is the model that 
# is fitted to the data, and we can see how well the trendline fits the data. We can see that there is a positive linear
